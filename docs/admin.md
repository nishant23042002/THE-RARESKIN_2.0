...
| `campaign` | The running offer ("Buy 1, get 1", a festive drop). Toggle **Show the offer card** on and fill the headline/detail; a small dismissible card appears bottom-left site-wide (`campaign-note.tsx`), riding above the bag bar and tucking away over the footer. Dismissal is per-visitor and keyed to **Offer code** — change the code for a new offer to bring the card back for everyone. Off by default; nothing to clean up pre-launch. |
